# -*- coding: utf-8 -*-
"""BLEU image_captioning.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1s50soel-RrlfIrusDpoIFfxuDChGw6NS
...
image_path_to_caption['/content/train2014/COCO_train2014_000000318556.jpg']
...
for j in range(len(img_name_val)):
  ...
  real_caption_clean = real_caption.replace('<start>', '').replace('<end>', '').replace('.', '').lower().strip()
  real_caption_split = real_caption_clean.split()
  ref_dict[image].append(real_caption_split)

for image in ref_dict.keys():
  result, attention_plot = evaluate(image)
  result_clean = [x for x in result if "." not in x and x !='<end>']
  hypotheses.append(result_clean)
...
#Image.open('/content/train2014/COCO_train2014_000000572109.jpg')

len(hypotheses)

corpus_bleu(references, hypotheses, weights=(1,))

corpus_bleu(references, hypotheses, weights=(1,))

corpus_bleu(references, hypotheses, weights=(.5,.5))

corpus_bleu(references, hypotheses, weights=(1/3, 1/3, 1/3,))
