...
The shape of the output of this layer is ```8x8x2048```. You use the last convolutional layer because you are using 
attention in this example. You don't perform this initialization during training because it could become a bottleneck.
...
Caching the output in RAM would be faster but also memory intensive, requiring 8 \* 8 \* 2048 floats per image. 
At the time of writing, this exceeds the memory limitations of Colab (currently 12GB of memory).

Performance could be improved with a more sophisticated caching strategy (for example, by sharding the images to reduce 
random access disk I/O), but that would require more code.
...
You will transform the text captions into integer sequences using the [TextVectorization]
(https://www.tensorflow.org/api_docs/python/tf/keras/layers/TextVectorization) layer, with the following steps:

* Use [adapt](https://www.tensorflow.org/api_docs/python/tf/keras/layers/TextVectorization#adapt) to iterate over all captions, 
split the captions into words, and compute a vocabulary of the top 5,000 words (to save memory).
...
img_list = ['https://commons.wikimedia.org/wiki/Category:Images_from_Mushroom_Observer#/media/File:2003-12-05_Russula_sanguinea_(Bull.)_Fr_112.jpg',
            'https://commons.wikimedia.org/wiki/Category:NARA_images_of_sports#/media/File:Ebbetts_Pass_Scenic_Byway_-_Powder_Run_at_Bear_Valley_-_NARA_-_7718117.jpg',
            'https://commons.wikimedia.org/wiki/Category:NARA_images_of_sports#/media/File:American_and_Soviet_sailors_run_in_a_track_meet_during_a_goodwill_visit_by_the_guided_missile_cruiser_USS_PRINCETON_(CG_59)_and_the_guided_missile_frigate_USS_REUBEN_JAMES_(FFG_57)_-_DPLA_-_b4e881c7cdf15a756b9ae1add2fec8ac.jpeg']
